...
## Introduction
We are solving the concurrent auction house with sockets between clients and servers
problem that was assigned to us as our final project in CS 351, design of large programs.
...
- Helped with Bank component in the collaborative effort of auction house communicating to bank a list
...
how to get the program stared, and how to actually use the program.

Also, its noted, that where ever the clients are run, it must have the same folder 'resources' in the same
directory. For example, you have folder: "AH", inside should be the jar's as well as the resources folder.
...
- Starting the communications between components
  ...
  * For Bank machine, you don't have have to change the configurations as you're not creating a client
  to another server
  * For Auction House, you must change the Agent & Bank's ip from local hose to the respective machines IP
  for that component.
  * For Agent, you must change the Auction Houses & Bank's IP to the respective machines IP for that component.
  * If you're running off of local hose on the same machine, verify all config files IP's are 'localhost'
  ...
  * You can now either launch the Auction Houses or Agent  java applications in any oder now. For purposes 
  of the usage section I will instruct on launching Auction Houses First.
    ...
    auction houses gui info. The ID will update, the balance, and the amount of items the Auction House has.
  ...
  * Lastly, comes launching the Agent java application.
    ...
    + You will then select the port number that agent will hose off of, default is 8900.
    + If everything launched properly you will see another display popup of the agent's main client.
    + You will see all sorts functionality that will be described in the next section of usage.
...
- Using the Agents Main GUI
  ...
  * You will see the sub sections on the left of this client
    ...
    + Agent Name displays the random generated First and Last name you were given
    + Agent Account display your account number (You don't see you're secret bidding key).
  ...
  * The next section you will see the labels, Item Name, Item ID, Starting Bid, Current Bid, a Text field that shows
  0.00 and last a button with the text place bid.
    + Item Name will display the item's description of the item you're viewing off the table. Default this is blank.
    + Item ID will list the item's ID that you're viewing off the table. Default this is blank.
    + Starting Bid is the minimum bid the user must set on the item to start the bidding for the item. Default
    this is blank.
    ...
    selected it will display the current bid of the item. Default this is blank, but when clicked on an item it always sets
  ...
  * The last section is a notification box.
    ...
    + You have a possibility of receiving four different messages
        ...
        + Reject Response
            ...
            the item, or lastly you have an item selected that the bidding already ended for.
        + Accept Response
  ...
  * The other half of the client.
    * Above the table you will see a label "Auction House" and to the right of it a combo box you can click on.
    * When clicking on the combo box, you will se a list of available auction houses that are currently open.
    * Simply left click on the Auction House you want to view. When doing this the Auction House will display it's
    ...
    * Once the timer expired, the Agent clients update to refresh this change, and remove the item.
...
## Project Assumptions
A few things were assumed during this project. 
* An auction item is only active when the first agent bids the minimum amount or more. But until then
the items remain there. This allows us for unscripted items going up for bid. Which could've been also easily
...
interacts ever with this account again. The account object still exists within the bank, but we have no access to it. This
...
* Names and account IDs don't really do a whole lote in terms of the bank component. It just assigns the account to hold the ID.
Everything in Bank's accounts it tied up through a hash map that uses the secret keys given as it key. I believe this
...
It's to my knowledge that we fully implemented Professor Roman's design instruction for the component needed
